...
const DEMO_DOMAIN = "wcdemo.local";
...
async function main() {
  ...
  if (qualifierIds.length !== 32) {
    console.warn(`⚠ Expected 32 qualifiers, found ${qualifierIds.length}. Scores may be off.`);
  }
  ...
  console.log(`Cleared ${deleted.count} previous demo users (and their cascaded data).`);

  const passwordHash = hashPassword(DEMO_PASSWORD);
  let userCounter = 0;

  const summary = { users: 0, groups: 0, qualPredictions: 0, r32Predictions: 0, scores: 0 };
  ...
  console.log("\n✅ Demo seed complete for:", tournament.name);
  console.log(`   Users:            ${summary.users}`);
  console.log(`   Groups:           ${summary.groups} (${GROUP_NAMES.join(", ")})`);
  console.log(`   Qual predictions: ${summary.qualPredictions} (scored)`);
  console.log(`   R32 predictions:  ${summary.r32Predictions} (submitted, awaiting results)`);
  console.log(`   Trivia questions: ${triviaCount} (today → ${TRIVIA.length - 1} days back)`);
  console.log(`\n   Demo login: any of demo1..demo${summary.users}@${DEMO_DOMAIN} / password "${DEMO_PASSWORD}"`);
}
